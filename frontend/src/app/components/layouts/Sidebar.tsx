...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-card text-card-foreground border-r border-border">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-border h-[69px]">
        ...
        {navItems.map((item) => {
          ...
          return (
            <motion.button
              ...
              whileHover={{ x: 4 }}
              whileTap={{ scale: 0.98 }}
              className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors cursor-pointer text-sm font-medium ${
                isActive
